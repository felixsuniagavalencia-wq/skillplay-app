...
export default function HomeScreen({ userId, onStartGame, onGoToWallet, onTopUp, onGoToProfile }: {
  ...
  const fetchBalance = async () => {
    try {
      ...
      setFreeGamesLeft(data.freeGamesLeft || 0);
    } catch (err) {}
  };

  const getDifficultyLabel = (diff: string) => {
    const key = `home_difficulty_${diff}` as any;
    return t(key);
  };

  const handleStartGame = async (isFree: boolean = false) => {
    if (!isFree && balance < entryFee) {
  ...
  return (
    <>
      ...
      <ScrollView style={styles.container}>
        ...
        {freeGamesLeft > 0 && (
          <TouchableOpacity style={styles.freeGameBanner} onPress={() => handleStartGame(true)}>
            <Text style={styles.freeGameIcon}>🎁</Text>
            <View>
              <Text style={styles.freeGameTitle}>{t('home_free_available')}</Text>
              <Text style={styles.freeGameSubtitle}>{t('home_free_remaining', { count: freeGamesLeft })}</Text>
            </View>
          </TouchableOpacity>
        )}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.row}>
          {DIFFICULTIES.map(diff => (
            <TouchableOpacity key={diff} style={[styles.chip, difficulty === diff && styles.chipActive]} onPress={() => setDifficulty(diff)}>
              <Text style={[styles.chipText, difficulty === diff && styles.chipTextActive]}>{getDifficultyLabel(diff)}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        {freeGamesLeft > 0 && (
          <TouchableOpacity style={styles.freeBtn} onPress={() => handleStartGame(true)} disabled={loading}>
            <Text style={styles.freeBtnText}>{t('home_play_free', { count: freeGamesLeft })}</Text>
          </TouchableOpacity>
        )}
      </ScrollView>
    </>
...
const styles = StyleSheet.create({
  ...
  freeGameBanner: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#064E3B', borderRadius: 12, padding: 14, marginBottom: 16, gap: 12, borderWidth: 1, borderColor: '#22C55E' },
  freeGameIcon: { fontSize: 28 },
  freeGameTitle: { color: '#22C55E', fontWeight: 'bold', fontSize: 15 },
  ...
  freeBtn: { backgroundColor: '#22C55E', borderRadius: 12, padding: 16, alignItems: 'center', marginBottom: 40 },
  freeBtnText: { color: 'white', fontSize: 16, fontWeight: 'bold' },
